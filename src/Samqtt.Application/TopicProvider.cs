...
namespace Samqtt.Application
{
    public class TopicProvider : ITopicProvider
    {
        ...
        /// </summary>
        private readonly string _appUniqueIdPrefix;
        ...
        public TopicProvider(IOptionsMonitor<SamqttOptions> options)
        {
            _deviceIdentifier = options.CurrentValue?.DeviceIdentifier.ToLowerInvariant()
                ?? throw new ArgumentNullException(nameof(options), "SamqttOptions.DeviceIdentifier cannot be null.");
            _appUniqueIdPrefix = $"{_appIdPrefix}_{_deviceIdentifier}";
            StatusTopic = $"{_appIdPrefix}/{_deviceIdentifier}/status";
            UnsubscribeTopic = $"{_appIdPrefix}/{_deviceIdentifier}/#";
        }
        ...
        /// <returns></returns>
        public string GetUniqueId(string name) => $"{_appUniqueIdPrefix}_{SanitizeHelpers.Sanitize(name)}";
        ...
        /// <returns></returns>
        public string GetBinarySensorDiscoveryTopic(string sensorName) => $"{Constants.HomeAssistantTopic}/{BinarySensorDomain}/{_appUniqueIdPrefix}_{SanitizeHelpers.Sanitize(sensorName)}/config";
        ...
        /// <param name="sensorName">Sensor name</param>
        /// <returns></returns>
        public string GetStandardSensorDiscoveryTopic(string sensorName) => $"{Constants.HomeAssistantTopic}/{SensorDomain}/{_appUniqueIdPrefix}_{SanitizeHelpers.Sanitize(sensorName)}/config";
        ...
        /// <param name="actionName">Action name</param>
        /// <returns></returns>
        public string GetActionResponseDiscoveryTopic(string actionName) => $"{Constants.HomeAssistantTopic}/{SensorDomain}/{_appUniqueIdPrefix}_{SanitizeHelpers.Sanitize(actionName)}/config";
        public string GetButtonDiscoveryTopic(string actionName) => $"{Constants.HomeAssistantTopic}/button/{_appUniqueIdPrefix}_{SanitizeHelpers.Sanitize(actionName)}/config";
